...
"""
        ...
        # Replace the placeholders manually to avoid format string issues
        formatted_prompt = prompt
        formatted_prompt = formatted_prompt.replace("{TARGET_FORM_NAME}", target_form_name)
        formatted_prompt = formatted_prompt.replace("{FIELD_MAPPING_TABLE}", field_mapping_table)
        formatted_prompt = formatted_prompt.replace("{FIELD_NAMES}", json.dumps(field_names, indent=2))
        formatted_prompt = formatted_prompt.replace("{NUMBER_TO_DESCRIPTION_MAP}", json.dumps(self.number_to_description_map, indent=2))
        formatted_prompt = formatted_prompt.replace("{SOURCE_DOCUMENTS}", text_context or "[Source documents will be processed by the LLM client]")
        
        logger.debug("✅ Manually replaced all placeholders in prompt")
